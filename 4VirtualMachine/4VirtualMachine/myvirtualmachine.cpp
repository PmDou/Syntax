...
Code::Code(const char* opt, int operand1, int operand2) {
	char t1[20], t2[20];
	//itoa(operand1, t1, 10);
	sprintf(t1, "%d", operand1);
	//itoa(operand2, t2, 10);
	sprintf(t2, "%d", operand2);
	strcpy(this->opt, opt);
...
enum Opt {
	NUL, LOADi, LOADIi, STO, ADD, SUB, MULT, DIV, BR, BRF, EQ, NOTEQ, GT, LES, GE, LE, AND, OR, INi, OUTi, RETURN,
		...
		// 注释或者除号识别
			...
			if ('*' == _words_[i]) {
				...
				while (flag && _words_[i]) {
					...
					else if (_words_[i] >= 0 && _words_[i] <= 255) {
						col++;
					}
					else {
						col++;
...
int factor(int depth, int* _size, VaribleType* type) {
	...
	if (equals(t.type, "ID")) {
		...
		if (NULL == sym) return err(t, 1, ("变量'" + string(t.token) + "'未定义\n").c_str());
		...
		if (equals(t.type, "[")) {
			...
			if (equals(t.type, "Integer")) {
				...
				if (NULL == sym2) return err(t, 1, ("变量'" + string(t.token) + "'未定义\n").c_str());
		...
		Symbol* sym = lookup(t.token, variable);
		if (NULL == sym) return err(t, 1, ("变量'" + string(t.token) + "未定义\n").c_str());
		if (data_type_size[sym->type] != sym->args) return err(t, 1, ("不可对数组名'" + string(t.token) + "'直接进行自增自减操作。\n").c_str());
		if (NULL != _size) *_size = sym->args;
		if (NULL != type) *type = data_type[sym->type];
		if (strstr(sym->type, "int"))
...
int call_stat(int depth, int* _size, VaribleType* type) {
	...
	if (NULL == sym) return err(t, 1, ("函数'" + string(t.token) + "'未定义。\n").c_str());
	if (NULL != _size) *_size = sym->args;
	if (NULL != type) *type = data_type[sym->type];
	if (0 != sym->args) {
		if (strstr(sym->type, "int"))
...
int main(int argc, char* argv[]) {
	...
	if (OUTPUT)
		printf("\n语法分析结束\n");
	printf("---------------------------------------------\n");
	printf("语义分析及中间代码生成开始\n\n");
	printf("语义分析及中间代码生成结束\n");
	printf("---------------------------------------------\n");
	printf("虚拟机开始\n");
